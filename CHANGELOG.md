...
- Document the role tags, check mode support (converged hosts only) and the package/service state variables in the README ([\#XXX](https://github.com/PowerDNS/dnsdist-ansible/pull/XXX))

BUG FIXES:
- Tag the tasks inside `install.yml`, `configure.yml` and `repo-*.yml` so that `--tags install`, `--tags config` and `--tags repository` no longer run the include and skip its body. A dynamic `include_tasks` does not pass its tags to the tasks it includes, so filtered runs silently did nothing and still exited 0 ([\#XXX](https://github.com/PowerDNS/dnsdist-ansible/pull/XXX))
- Add `check_mode: false` to the control socket key probe so `--check` against a converged host no longer reports the configuration file as changed with the existing `setKey(...)` line removed ([\#XXX](https://github.com/PowerDNS/dnsdist-ansible/pull/XXX))
