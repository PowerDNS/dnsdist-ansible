...
BUG FIXES:
- Correct the documentation of which configuration file the daemon reads in `yaml` format. Auto-detection of `dnsdist.yml` starts with dnsdist 2.1: given both files a 2.1 reads the `.yml`, while a 2.0 reads `dnsdist.conf` and never looks at the `.yml` unless `--config` names it. The role claimed no unit override was needed for a single instance, so a 2.0 host in `yaml` format kept running the packaged Lua configuration and silently ignored the YAML the role wrote - both files parse, and the role's own configuration check passes against the name it chooses. The README now documents the `ExecStart` and `ExecStartPre` overrides a 2.0 needs, and the YAML leg of the `default` Molecule scenario runs only on the releases that auto-detect the file ([\#200](https://github.com/PowerDNS/dnsdist-ansible/pull/200))
