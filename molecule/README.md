...
when it needs a service account.
...
| set | contents | leg |
|---|---|---|
| `dnsdist-<newest>` | every operating system that release has packages for, from Enterprise Linux 9 upwards | `ansible220` |
| `dnsdist-<older>` | one RPM host and one DEB host of that release | `ansible220` |
...
| `dnsdist-<newest>-ansible218` | one platform | `ansible218` only |
| `dnsdist-<newest>-ansible216-single` | one Enterprise Linux 8 host | `ansible216`, `multi-instance` only |
...
3. **Four new platform sets.** Check availability first, then list exactly what
   ...
   - `molecule/platforms/dnsdist-22.yml` - the full operating system matrix, which
     only the newest release carries: Enterprise Linux 9 upwards, Debian and Ubuntu
     22.04 upwards. Copy the set of the release that was newest until now,
   ...
   - `molecule/platforms/dnsdist-22-ansible218.yml` - one platform, Debian 13.
   - `molecule/platforms/dnsdist-22-ansible216-single.yml` - one Enterprise Linux 8
     host.

4. **Narrow the release that was newest until now.** In `dnsdist-21.yml`, comment
   out every platform except one RPM host and one DEB host, keeping the reason in
   the file, and delete `dnsdist-21-ansible218.yml` and
   `dnsdist-21-ansible216-single.yml`: those two only ever apply to the newest
   release.
5. **`.github/workflows/main.yml`** - add one row per set:
   ...
   Repoint the single `ansible218` row at `dnsdist-22-ansible218`.

6. **Move `multi-instance` to the new release** by changing `version: "21"` to
   `version: "22"` in its two matrix rows, and the 2.16 one to
   `dnsdist-22-ansible216-single`. That scenario only ever runs against the newest
   release.
...
8. **Default for local runs** - `molecule/resources/vars/dnsdist-version.yml`
